...
def run_emailharvester(email: str, output_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raw_output, status, data = f"Ferramenta não encontrada no PATH: {exc}", "unavailable", {"error": str(exc), "domain": dominio, "api_keys": {key: "configured" for key in sorted(api_config)}}
    except subprocess.TimeoutExpired as exc:
        raw_output, status, data = f"Execução excedeu o timeout de 120 segundos: {exc}", "timeout", {"error": str(exc), "domain": dominio, "api_keys": {key: "configured" for key in sorted(api_config)}}
    except OSError as exc:
        raw_output, status, data = f"Falha ao iniciar ferramenta: {exc}", "error", {"error": str(exc), "domain": dominio, "api_keys": {key: "configured" for key in sorted(api_config)}}
