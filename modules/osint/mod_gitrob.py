"""Adaptador independente para o Gitrob."""
from pathlib import Path
import subprocess
from typing import Any


def run_gitrob(email: str, output_dir: Path) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / "gitrob.txt"
    try:
        result = subprocess.run(["gitrob", "--email", email], capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=180, check=False)
